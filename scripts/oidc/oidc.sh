...
SUBSCRIPTION_NAME=$(echo "${SUBSCRIPTION}" | jq -r .name | tr -d '\r')
SUBSCRIPTION_ID=$(echo "${SUBSCRIPTION}" | jq -r .id | tr -d '\r')
TENANT_ID=$(echo "${SUBSCRIPTION}" | jq -r .tenantId | tr -d '\r')
...
APP_NAME=$(echo "${CONFIG}" | jq -r .appName | tr -d '\r')
...
for FIC in "${fics[@]}"; do
  FIC_NAME=$(echo "${FIC}" | jq -r .name | tr -d '\r')
  ...
  SUBJECT=$(echo "${FIC}" | jq -r .subject | tr -d '\r')
...
for ROLE_ASSIGNMENT in "${ROLE_ASSIGNMENTS[@]}"; do
  ROLE=$(echo "${ROLE_ASSIGNMENT}" | jq -r .role | tr -d '\r')
  SCOPE=$(echo "${ROLE_ASSIGNMENT}" | jq -r .scope | tr -d '\r')
  CONDITION=$(echo "${ROLE_ASSIGNMENT}" | jq -r .condition | tr -d '\r')
